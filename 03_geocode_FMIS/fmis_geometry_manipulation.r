####Setup####
  ...
  ##main route geometry from NHPN constructed routes (matched by state + sign type + route number)
    nhpn_dir <- '/Users/fm557/YLS Dropbox/Finn Meffe/FHWA cost data/Data/Intermediate/NHPN_routes'
    nhpn_routes <- do.call(rbind, lapply(
      c('nhpn_interstate_by_sign', 'nhpn_us_route_by_sign', 'nhpn_state_route_by_sign'),
      function(f) read_sf(file.path(nhpn_dir, paste0(f, '.gpkg'))))) %>%
      st_transform(4326) %>%
      mutate(stfips   = str_pad(stfips, 2, 'left', '0'),
             join_num = str_remove(str_trim(signn), '^0+'))

    main_routes_intermediate2 <- read_sf(paste0('main_routes_intermediate/main_routes_intermediate_', version, '.gpkg')) %>%
      st_drop_geometry() %>%
      mutate(join_num = str_remove(str_trim(main_route_num), '^0+')) %>%
      left_join(nhpn_routes %>% select(stfips, signt, join_num, geom),
                by = c('state' = 'stfips', 'rttyp' = 'signt', 'join_num')) %>%
      st_as_sf() %>%
      mutate(is_geom_empty = st_is_empty(geom)) %>%
      select(-join_num)
