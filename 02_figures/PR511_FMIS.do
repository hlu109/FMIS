...
drop if authconstyear < 1950 | authconstyear > 1989
sort authconstyear
...
* compute pseudo mileage 
preserve
use "$intermediate_data/PR511_hubbardmazzeo.dta", clear
rename open_year pr_year
drop if mi(pr_year)
collapse (sum) pr511_mi = seg_len, by(pr_year)
...
forvalues lag = 0/5 {
    ...
    merge m:1 pr_year using `pr511_mi_by_yr', keep(master match) nogen keepusing(pr511_mi)
    ...
    graph export "$out_dir/duration_per_pseudo_mi_by_authyr_lag`lag'.png", replace width(2400)
    restore
}
