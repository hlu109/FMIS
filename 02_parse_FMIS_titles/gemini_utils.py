...
def log_config(prompt_text_path, gemini_model_id, identifier, log_dir,
               input_path=None, row_indices=None):
    ...
    with open(file_path, "a", encoding="utf-8") as file:
        ...
        file.write(f"Row indices filter: {row_indices if row_indices else 'All rows'}\n")
...
def process_titles(genai_client,
    ...
    for i, (idx, row) in enumerate(df_subset.iterrows()):
        ...
        if os.path.exists(json_path):
            ...
            row_df["model_id"] = result_json.get("model_id", model_id)
        ...
        project_input = {
            ...
            "state_fips": int(row["state_fips"]) if pd.notna(row.get("state_fips")) else None,
            "state_name": str(row.get("state_name", "")), # TODO: extract this in FMIS title .do file 
            "county_fips": int(row["county_fips"]) if pd.notna(row.get("county_fips")) else None,
            "county_name": str(row.get("county_name", "")), # TODO: extract this in FMIS title .do file 
        ...
        try:
            ...
            if all_dataframes:
                ...
                _log_and_print(
                    f"Partial results saved to {outfile_path} ({partial_df.shape[0]} rows) "
                    f"before fatal rate limit stop.",
                    log_dir, identifier)
            raise
