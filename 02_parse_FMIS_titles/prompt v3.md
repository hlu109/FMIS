...
Do not include terms like "interchange", "junction", or "intersection" — these are implicit in the coding.
...
- `state_line`: state boundary (e.g., S.L., STATE LINE, S/L). Also code international boundaries as a state boundary.
...
- `other_terrain`: natural terrain feature not covered above.
- `other_landmark`: built facility not covered above (toll plaza, weigh station, rest area, etc.)
