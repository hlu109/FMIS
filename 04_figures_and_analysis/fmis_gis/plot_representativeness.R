...
sample_2015_gis <- plf_gis %>% filter(completion_year >= 2015, has_gis == 1)
...
state_rep_scatter <- function(
  ...
  lim <- c(
    0,
    max(dplyr::pull(data, {{ x }}), dplyr::pull(data, {{ y }}), na.rm = TRUE)
  )
  ggplot(data, aes({{ x }}, {{ y }})) +
...
state_rep_proj_scatter <- state_rep_scatter(
  ...
  axis_unit = "projects",
  label = abbr,
  caption = "Each point is a state, with axis representing that state's share of projects
...
corr_ncodes_gis_share <- ggplot(
  data = make_state_df(sample_2015),
  aes(x = avg_detail_codes, y = pct_gis, label = abbr)
) +
  geom_point() +
  ggrepel::geom_text_repel(
    size = 2.4,
...
type_rep_bar <- function(
  ...
  ggplot(long, aes(x = work_type, y = share, fill = sample)) +
    ...
    scale_fill_manual(
      values = c("Full 2015-24 sample" = "grey65", "GIS subsample" = "#2c7fb8"),
      name = NULL
    ) +
    labs(
      title = title,
...
code_rep_bar <- function(
  ...
  long <- top %>%
    ...
    mutate(
      code_label = factor(
        str_wrap(code_label, 24),
        levels = str_wrap(lvls, 24)
      ),
      sample = factor(
        sample,
        levels = c("full", "gis"),
...
code_rep_proj_scatter <- state_rep_scatter(
  ...
  axis_unit = "projects",
  label = code,
  caption = "Each point is a FHWA improvement type code, with axis representing that state's share of projects
  in the respective full (all 2015-2024 FMIS data) or limited (has mile markers) sample. 
  Observations above the line are overrepresented
  in the \"has milemarker\" subsample; observations under the line are underrepresented."
)
