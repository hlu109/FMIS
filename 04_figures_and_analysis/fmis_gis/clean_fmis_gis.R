...
make_plf_gis <- function(plf_lite_adj, sample = c("completion", "authorization", "original")) {
  ...
  out <- if (sample == "original") {
    ...
    out %>% filter(auth_year <= 2025,
                   completion_year <= 2025) # both completion and auth exist, completion before 2025
  }

  out %>%
    mutate(num_detail_codes = str_count(proj_improv_types, ";") + 1,
           gis_length = gis_endpoint - gis_beginpoint,
           log_cost_mile = ifelse(gis_length == 0 | is.na(gis_length) | total_cost_mills == 0,
                                  NA, log(total_cost_mills / gis_length)))
}
...
plf_gis_cy_iijacodes <- make_plf_gis(plf_lite_cy %>% filter(detail_prefix %in% iija_codes),
                                     "completion")
plf_gis_ay_iijacodes <- make_plf_gis(plf_lite_ay %>% filter(detail_prefix %in% iija_codes),
                                     "authorization")
plf_gis_cy_1yr_iijacodes <- make_plf_gis(plf_lite_cy %>% filter(detail_prefix %in% iija_codes,
                                                                completedate - authconstdate <= 365),
                                         "completion")
plf_gis_cy_2yr_iijacodes <- make_plf_gis(plf_lite_cy %>% filter(detail_prefix %in% iija_codes,
                                                            completedate - authconstdate <= 730),
                                     "completion")
