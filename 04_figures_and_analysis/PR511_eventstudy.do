...
merge m:1 county_fips using "$pr511_intermediate/pr511_cty_single_chain_ever", keep(3) nogen
...
save "$intermediate_data/PR511_FMIS_eventstudy_cty_single_chain_ever.dta", replace
...
save "$intermediate_data/PR511_FMIS_eventstudy_cty_single_openyr_ever.dta", replace
...
use "$intermediate_data/PR511_FMIS_eventstudy_cty_single_chain_ever.dta", clear
local costs total_cost_mills_adj new_construction_cost_mills_adj row_cost_mills_adj pe_cost_mills_adj
foreach cost of local costs {
...
* ==============================================================================
* Event study for county x routes with exactly one PR-511 year ever
* ==============================================================================
...
rename st state_fips
rename county countyid
keep state_fips countyid route open_year
duplicates drop
tempfile pr511_one_year_cty_rt
save `pr511_one_year_cty_rt'

* merge with FMIS cost data
use "$intermediate_data/receipt_level_FMIS.dta", clear
keep if funding_program == "Interstate Construction"
keep if completion_year <= 2000 & completion_year >= 1950
* extract route from the federal project number (route_fpn) to match PR-511 route
...
keep state_fips countyid county_fips route federal_project_number completedate completion_year total_cost_mills detail_improvementtype new_construction
...
* merge with PR-511 data
merge m:1 state_fips countyid route using `pr511_one_year_cty_rt', keep(3) nogen

* compute event time
gen event_time = completion_year - open_year

* aggregate FMIS spending by year, state, county x route
collapse (sum) total_cost_mills new_construction_cost row_cost pe_cost (firstnm) event_time state_fips countyid, by(county_fips route open_year completion_year)
...
egen county_fips_route_id = group(county_fips route)
drop if missing(event_time)

tsset county_fips_route_id event_time
tsfill, full
...
save "$intermediate_data/PR511_FMIS_eventstudy_cty_rt_single_openyr_ever.dta", replace

* ==============================================================================
* Plot event study

* compute number of county x routes to print in graph
preserve
bysort county_fips route: keep if _n == 1
local n_cty_rt = _N
restore
...
twoway ///
    ...
    xline(0, lpattern(dot)) ///
    title("Average spending in FMIS Interstate Construction" "around PR-511 opening", size(medsmall)) ///
    subtitle("For county x routes with exactly one year of PR-511 data ever", size(vsmall)) ///
    xtitle("Years after PR-511 opening", size(small)) ///
    ytitle("Millions of 2025 USD", size(small)) ///
    xlabel(-40(5)40, labsize(small)) ///
    ylabel(, labsize(small) angle(horizontal) format(%9.1f)) ///
    note( ///
        ...
        `"Interstate receipts are identified by the "Interstate Construction" funding program code."' ///
        "FMIS data from 1950 to 2000." ///
        "Number of county x routes: `n_cty_rt'.", ///
         size(vsmall) span ///
    ) ///
    legend(off) ///
    ysize(4) xsize(6)
graph export "$out_dir/pr511_IC_eventstudy_totalcost_avg_cty_rt_oneyear.png", replace width(2400)
...
twoway ///
    ...
    legend(off) ///
    ysize(4) xsize(6)
graph export "$out_dir/pr511_IC_eventstudy_newconstr_avg_cty_rt_oneyear.png", replace width(2400)
...
twoway ///
    ...
    legend(off) ///
    ysize(4) xsize(6)
graph export "$out_dir/pr511_IC_eventstudy_rowcost_avg_cty_rt_oneyear.png", replace width(2400)
...
twoway ///
    ...
    legend(off) ///
    ysize(4) xsize(6)
graph export "$out_dir/pr511_IC_eventstudy_pecost_avg_cty_rt_oneyear.png", replace width(2400)
...
twoway ///
    ...
    xline(0, lpattern(dot)) ///
    title("Average spending in FMIS Interstate Construction" "around PR-511 opening", size(medsmall)) ///
    subtitle("For county x routes with exactly one year of PR-511 data ever" "Event time −5 to +5 years", size(vsmall)) ///
    xtitle("Years after PR-511 opening", size(small)) ///
    ytitle("Millions of 2025 USD", size(small)) ///
    xlabel(-5(1)5, labsize(small)) ///
    ylabel(, labsize(small) angle(horizontal) format(%9.1f)) ///
    ...
    legend(off) ///
    ysize(4) xsize(6)
graph export "$out_dir/pr511_IC_eventstudy_totalcost_avg_cty_rt_oneyear_etm5p5.png", replace width(2400)
...
twoway ///
    ...
    legend(off) ///
    ysize(4) xsize(6)
graph export "$out_dir/pr511_IC_eventstudy_newconstr_avg_cty_rt_oneyear_etm5p5.png", replace width(2400)
...
twoway ///
    ...
    legend(off) ///
    ysize(4) xsize(6)
graph export "$out_dir/pr511_IC_eventstudy_rowcost_avg_cty_rt_oneyear_etm5p5.png", replace width(2400)
...
twoway ///
    ...
    legend(off) ///
    ysize(4) xsize(6)
graph export "$out_dir/pr511_IC_eventstudy_pecost_avg_cty_rt_oneyear_etm5p5.png", replace width(2400)
