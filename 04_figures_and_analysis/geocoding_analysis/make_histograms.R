state_proj_hist <- ggplot(data = plf_states, mapping = aes(x = pct_proj_geo)) +
  geom_histogram(aes(y = after_stat(count) / sum(after_stat(count))),
                     bins = 20, fill = "#4292c6", color = "white", linewidth = 0.2) +
  ...
  labs(title = "Distribution of State-level Geocoding Rates",
       ...
       caption = append_n_to_caption("", sum(!is.na(plf_states$pct_proj_geo)),
                                     "state", "states")) +
  qje_theme()
...
state_spend_hist <- ggplot(data = plf_states, mapping = aes(x = pct_spending_geo)) +
  ...
  labs(title = "Distribution of State-level Geocoding Rates",
       ...
       caption = append_n_to_caption("", sum(!is.na(plf_states$pct_spending_geo)),
                                     "state", "states")) +
  qje_theme()
...
county_proj_hist <- ggplot(data = plf_counties, mapping = aes(x = pct_proj_geo)) +
  ...
  labs(title = "Distribution of County-level Geocoding Rates",
       subtitle = "Share of interstate projects successfully geocoded",
       x = "Share of projects geocoded", y = "Percentage",
       caption = append_n_to_caption("", sum(!is.na(plf_counties$pct_proj_geo)),
                                     "county", "counties")) +
  qje_theme()
...
county_spend_hist <- ggplot(data = plf_counties, mapping = aes(x = pct_spending_geo)) +
  ...
  labs(title = "Distribution of County-level Geocoding Rates",
       subtitle = "Share of interstate spending successfully geocoded",
       x = "Share of spending geocoded", y = "Percentage",
       caption = append_n_to_caption("", sum(!is.na(plf_counties$pct_spending_geo)),
                                     "county", "counties")) +
  qje_theme()
...
make_county_hist_decade <- function(data, geo_var, title, subtitle, x_lab, time_var) {
...
county_proj_hist_decade <- make_county_hist_decade(
  plf_counties_decade, pct_proj_geo,
...
county_spend_hist_decade <- make_county_hist_decade(
  plf_counties_decade, pct_spending_geo,
  title    = "Distribution of County-level Geocoding Rates",
