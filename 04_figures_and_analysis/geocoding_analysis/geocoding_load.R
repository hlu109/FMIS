...
project_level_fmis <- read_dta(file.path(
  data_dir,
...
receipt_level_fmis <- read_dta(file.path(
  data_dir,
  "receipt_level_FMIS_lite.dta"
)) %>%
  mutate(
    project_id = paste0(
      sprintf("%02d", recipientid),
...
project_level_fmis_geo <- fread(file.path(
  data_dir,
  "geocoding",
  "title_parsing_gemini_output",
  oneyear_counties_subset
)) %>%
  select(
    recipient_id,
    federal_project_number,
  ...
  mutate(
    ...
    ep_a_max_precision = ifelse(
      is.na(ep_a_max_precision),
      0,
      ep_a_max_precision
    ),
    ep_b_max_precision = ifelse(
      is.na(ep_b_max_precision),
      0,
      ep_b_max_precision
    ),
    geocoded = ifelse(
...
plf_join <- project_level_fmis_geo %>%
  ...
  filter(
    funding_program == "Interstate Construction",
    completion_year >= 1950 & completion_year <= 2000,
    detail_improvementtype != 5 | detail_improvementtype != 59
  ) %>%
  mutate(
    new_construction_cost = if_else(
      new_construction == 1,
      total_cost_mills,
      NA_real_
    ),
    reconstruction_cost = if_else(
      reconstruction == 1,
      total_cost_mills,
      NA_real_
    ),
    rehabilitation_cost = if_else(
      rehabilitation == 1,
      total_cost_mills,
      NA_real_
    ),
...
write.csv(
  plf_join,
  file = file.path(data_dir, "geocoding/project_level_FMIS_geocoded.csv"),
  row.names = F
)
