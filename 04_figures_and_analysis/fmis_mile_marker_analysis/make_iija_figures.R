iija_apportion <- read_xlsx(
  file.path(data_dir, "fmis_mile_markers", "apportionments_2022_2026.xlsx"),
  sheet = "FY 2021 & Est FY 2022-FY 2026",
  range = "B15:E65",
  col_names = c("state", "app_2021", "blank", "app_2022")
) %>%
  select(-blank) %>%
  mutate(
    iija_shock = (app_2022 - app_2021),
    iija_shock_pct = iija_shock / app_2021,
    abbr = state.abb[match(state, state.name)]
  ) %>%
  inner_join(
    state_pop <- read_csv(file.path(
      data_dir,
      "fmis_mile_markers",
      "state_pop_timeseries_fred.csv"
    )) %>%
      filter(year == 2021) %>%
      select(-year),
    by = join_by(abbr == state)
  ) %>%
  mutate(iija_shock_pc = iija_shock / pop, app_2021_pc = app_2021 / pop)

make_year_comp_df <- function(data) {
  data %>%
    filter(
      !is.na(authconstdate), # double check on cy and ay data, the base plf_gis shouldn't go thorugh here
      !is.na(log_cost_mile)
    ) %>%
    group_by(state_fips) %>%
    summarise(
      avg_cost_2015_2021 = mean(log_cost_mile[
        year(authconstdate) >= 2015 & year(authconstdate) <= 2021
      ]),
...
make_year_comp_scatter <- function(
  data,
  ...
  n_dropped <- sum(
    is.nan(df$avg_cost_2015_2021) | is.nan(df$avg_cost_2022_2025)
  )
  ...
  p <- ggplot(
    df,
    aes(x = avg_cost_2015_2021, y = avg_cost_2022_2025, label = abbr)
  ) +
    geom_point() +
    ...
    coord_fixed(ratio = 1) +
    labs(
      title = "State-level Average log Cost per Mile, 2015-2021 vs. 2022-2025",
      subtitle = str_glue(
        "FMIS Projects with Construction Authorization between 2015-2025 {sample_label}, {type_label}"
      ),
...
prep_iija_reg_df <- function(data, ..., start_year = 2015, end_year = 2025) {
  data %>%
    ...
    mutate(
      auth_year = lubridate::year(authconstdate),
      post2022 = as.integer(auth_year >= 2022)
    ) %>%
    filter(auth_year >= start_year, auth_year <= end_year) %>%
    inner_join(select(state_xwalk, state_fips, abbr), by = "state_fips") %>%
    inner_join(select(iija_apportion, abbr, iija_shock_pc), by = "abbr")
}
    ...
    error = function(e) {
      warning(
        str_glue(
          ...
          "refitting without fixed effects."
        ),
        call. = FALSE
      )
      tryCatch(
...
run_iija_did <- function(
  ...
  ic_terms <- if (length(ic_cols)) {
    paste0(" + ", paste(ic_cols, collapse = " + "))
  } else {
    ""
  }
...
plot_iija_eventstudy <- function(
  ...
  ggplot(es, aes(year, estimate)) +
    ...
    labs(
      title = "Event Study: Per-Capita IIJA Shock and log Cost per Mile",
...
build_iija_outputs <- function(data, sample_label, sample_note, sample_short) {
  scatters <- purrr::imap(
    iija_cost_types,
    ~ make_year_comp_scatter(
      filter_cost_type(data, .x),
      .y,
...
count_pre_post <- function(data, col) {
  ...
  tibble::tibble(pre = sum(df$post2022 == 0L), post = sum(df$post2022 == 1L))
}
...
pre_post_counts <- dplyr::bind_rows(
  ...
  dplyr::select(
    sample,
    as.vector(rbind(
      paste0(names(iija_cost_types_short), "__pre"),
...
print_pre_post_counts <- function(
  tbl = pre_post_counts,
  sample_width = "2.8cm"
) {
