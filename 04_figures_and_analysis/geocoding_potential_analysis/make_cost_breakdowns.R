...
plf_maj_flags <- plf_join %>%
  mutate(
    county_list = map(
      str_split(as.character(county_fips), ";"),
      clean_counties
    ),
...
make_costs_breakdown <- function(df, subset_flag, subset_label) {
  df %>%
    ...
    mutate(
      pct = 100 * cost / total_cost_mills,
      maj_spending = factor(
        maj_spending,
        labels = c("All other projects", subset_label)
      ),
      cost_type = factor(
        cost_type,
        levels = cost_types,
...
cost_break_bar_only <- ggplot(
  data = plf_counties_costs_only,
  ...
  labs(
    ...
    caption = paste0(
      ...
      nrow(plf_maj_flags),
      " total projects, ",
      nrow(plf_maj_flags %>% filter(only_maj == F)),
      " LHS projects, ",
      nrow(plf_maj_flags %>% filter(only_maj == T)),
      " RHS projects"
    )
  ) +
...
cost_break_bar_any <- ggplot(
  data = plf_counties_costs_any,
  aes(x = maj_spending, y = pct, fill = cost_type)
) +
  geom_col() +
  geom_text(
  ...
  labs(
    ...
    caption = paste0(
      ...
      nrow(plf_maj_flags),
      " total projects, ",
      nrow(plf_maj_flags %>% filter(any_maj == F)),
      " LHS projects, ",
      nrow(plf_maj_flags %>% filter(any_maj == T)),
      " RHS projects"
    )
  ) +
  scale_fill_brewer(palette = "Blues") +
...
cost_break_ungeocoded_df <- make_costs_breakdown_adj(
  plf_maj_flags %>%
    filter(geocoded == 0, any_maj == 1),
  any_maj
)

cost_break_ungeocoded <- ggplot(
  data = cost_break_ungeocoded_df,
  ...
  labs(
    ...
    caption = paste0(
      ...
      " ",
      nrow(plf_maj_flags %>% filter(geocoded == 0, any_maj == 1)),
      " total projects."
    )
  ) +
  scale_fill_brewer(palette = "Blues") +
...
create_by_type <- function(data) {
  map_dfr(cost_types, function(ct) {
    ...
    tibble(
      cost_type = ct,
      group = c(
        "Cost type not present",
        "Cost type present",
        "Standalone cost type"
      ),
...
geo_by_type_bar <- ggplot(
  ...
  geom_text(
    aes(
      label = sprintf("%.0f%%", 100 * pct_geocoded), #y = ci_upper
    ),
...
ngeo_by_type_bar <- ggplot(
  ...
  geom_text(
    aes(
      label = sprintf("%.0f%%", 100 * pct_not_geocoded), #y = ci_upper
    ),
    position = position_dodge(width = 0.8),
    vjust = -0.5,
    size = 2.5
...
geo_spending_by_type_bar <- ggplot(
  geo_by_type,
  aes(x = cost_type, y = pct_spending_geocoded, fill = group)
) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(
    aes(label = sprintf("%.0f%%", 100 * pct_spending_geocoded)),
    position = position_dodge(width = 0.8),
    vjust = -0.5,
    size = 2.5
  ) +
...
ngeo_spending_by_type_bar <- ggplot(
  geo_by_type,
  aes(x = cost_type, y = pct_spending_not_geocoded, fill = group)
) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(
    aes(label = sprintf("%.0f%%", 100 * pct_spending_not_geocoded)),
    position = position_dodge(width = 0.8),
    vjust = -0.5,
    size = 2.5
  ) +
...
# same breakdown, but weighting by spending (share of dollars geocoded)
# rather than by project count
geo_spending_by_type_bar_75any <- ggplot(
  geo_by_type_75any,
  aes(x = cost_type, y = pct_spending_geocoded, fill = group)
) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
...
ngeo_spending_by_type_bar_75any <- ggplot(
  geo_by_type_75any,
  aes(x = cost_type, y = pct_spending_not_geocoded, fill = group)
) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
...
structure_geo_by_type_bar <- ggplot(
  structure_geo,
  aes(x = structure_type, y = pct_geocoded, fill = group)
) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(
    aes(label = sprintf("%.0f%%", 100 * pct_geocoded)),
    position = position_dodge(width = 0.8),
    vjust = -0.5,
    size = 2.5
  ) +
  ...
  labs(
    ...
    caption = paste0(
      ...
      nrow(
        plf_geo %>% filter(road_cost > 0 | bridge_cost > 0 | tunnel_cost > 0)
      ),
      " total projects. ",
      nrow(plf_geo %>% filter(road_cost > 0)),
      " projects with roadway construction, ",
      nrow(plf_geo %>% filter(bridge_cost > 0)),
      " projects with bridge construction, ",
      nrow(plf_geo %>% filter(tunnel_cost > 0)),
      " projects with tunnel construction."
    )
  ) +
  scale_fill_brewer(palette = "Blues") +
  qje_theme()
...
structure_geo_spending_by_type_bar <- ggplot(
  structure_geo,
