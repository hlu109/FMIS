plf_join <- read_csv(file.path(
  data_dir,
  "geocoding/project_level_FMIS_geocoded.csv"
))
...
plf_states <- plf_join %>%
  ...
  inner_join(
    us_map(regions = "states", exclude = "PR"),
...
plf_counties <- plf_join %>%
  ...
  right_join(
    us_map(regions = "counties", exclude = "PR"),
...
one_pr511_year_counties <- read_dta(file.path(
  data_dir,
  "one_year_counties.dta"
)) %>%
  mutate(
    county_fips = paste0(
      sprintf("%02d", st),
