...
build_splits <- function(source, cluster_key_df, val_frac = 0.5, seed_salt = source,
  ...
  cluster_split <- tibble(
    ...
    split = ifelse(cluster_hash_frac(clusters, seed_salt) < val_frac, "val", "test")
  )
  ...
  train_rows <- tibble(projectid = train_ids, split = "train", cluster_key = NA_character_)
  return(bind_rows(labeled, train_rows))
}

build_splits_stratified <- function(
  source, cluster_key_df, strata_cols, val_frac = 0.5, seed_salt = source,
  universe_projectids = NULL) {
  ...
  per_stratum <- lapply(strata, function(s) {
    ...
    build_splits(
      source, cluster_key_df %>% filter(cluster_key %in% keys_in_stratum),
      val_frac = val_frac, seed_salt = paste(seed_salt, s, sep = "_")
    )
  })
