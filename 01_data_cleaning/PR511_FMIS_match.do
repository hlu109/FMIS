...
keep if state_fips == `state'
    ...
    * filter by year window and sort by time gap  
    ...
    save "$match_dir/PR511_FMIS_match_`state_name_file'${match_suffix}.dta", replace
...
if `has_pr_unmatched' == 1 {
    ...
    save "$match_dir/unmatched_PR511${match_suffix}.dta", replace
}
else {
...
if `has_fmis_unmatched' == 1 {
    ...
    save "$match_dir/unmatched_FMIS${match_suffix}.dta", replace
}
else {
...
local state_match_files : dir "$match_dir" files "PR511_FMIS_match_*${match_suffix}.dta"
